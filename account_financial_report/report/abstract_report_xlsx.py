...
class AbstractReportXslx(models.AbstractModel):
    ...
    # __slots__ = ['env', '_ids', '_prefetch_ids', 'sheet']
    ...
    #     self.env.columns = None

    #     # row_pos must be incremented at each writing lines
    #     self.env.row_pos = None

    #     # Formats
    #     self.env.format_right = None
    #     self.env.format_left = None
    #     self.env.format_right_bold_italic = None
    #     self.env.format_bold = None
    #     self.env.format_header_left = None
    #     self.env.format_header_center = None
    #     self.env.format_header_right = None
    #     self.env.format_header_amount = None
    #     self.env.format_amount = None
    #     self.env.format_percent_bold_italic = None
    ...
    def generate_xlsx_report(self, workbook, data, objects):
        ...
        self.env.columns = self._get_report_columns(report)
        self.env.workbook = workbook
        self.env.sheet = workbook.add_worksheet(report_name[:31])
    ...
    def _define_formats(self, workbook):
        ...
        """
        self.env.format_bold = workbook.add_format({"bold": True})
        self.env.format_right = workbook.add_format({"align": "right"})
        self.env.format_left = workbook.add_format({"align": "left"})
        self.env.format_right_bold_italic = workbook.add_format(
            {"align": "right", "bold": True, "italic": True}
        )
        self.env.format_header_left = workbook.add_format(
            {"bold": True, "border": True, "bg_color": "#FFFFCC"}
        )
        self.env.format_header_center = workbook.add_format(
            {"bold": True, "align": "center", "border": True, "bg_color": "#FFFFCC"}
        )
        self.env.format_header_right = workbook.add_format(
            {"bold": True, "align": "right", "border": True, "bg_color": "#FFFFCC"}
        )
        self.env.format_header_amount = workbook.add_format(
            {"bold": True, "border": True, "bg_color": "#FFFFCC"}
        )
        currency_id = self.env["res.company"]._get_user_currency()
        self.env.format_header_amount.set_num_format(
            "#,##0." + "0" * currency_id.decimal_places
        )
        self.env.format_amount = workbook.add_format()
        self.env.format_amount.set_num_format(
            "#,##0." + "0" * currency_id.decimal_places
        )
        self.env.format_amount_bold = workbook.add_format({"bold": True})
        self.env.format_amount_bold.set_num_format(
            "#,##0." + "0" * currency_id.decimal_places
        )
        self.env.format_percent_bold_italic = workbook.add_format(
            {"bold": True, "italic": True}
        )
        self.env.format_percent_bold_italic.set_num_format("#,##0.00%")

    def _set_column_width(self):
        """Set width for all defined columns.
        Columns are defined with `_get_report_columns` method.
        """
        for position, column in self.env.columns.items():
            self.env.sheet.set_column(position, position, column["width"])

    def _write_report_title(self, title):
        """Write report title on current line using all defined columns width.
        Columns are defined with `_get_report_columns` method.
        """
        self.env.sheet.merge_range(
            self.env.row_pos,
            0,
            self.env.row_pos,
            len(self.env.columns) - 1,
            title,
            self.env.format_bold,
        )
        self.env.row_pos += 3
    ...
    def write_array_header(self):
        ...
        for col_pos, column in self.env.columns.items():
            self.env.sheet.write(
                self.env.row_pos,
                col_pos,
                column["header"],
                self.env.format_header_center,
            )
        self.env.row_pos += 1
        ...
        for col_pos, column in self.env.columns.items():
            ...
            if cell_type == "many2one":
                self.env.sheet.write_string(
                    self.env.row_pos, col_pos, value.name or "", self.env.format_right
                )
                ...
                if (
                    ...
                    self.env.sheet.write_string(
                        self.env.row_pos, col_pos, value or "", self.env.format_bold
                    )
                else:
                    self.env.sheet.write_string(self.env.row_pos, col_pos, value or "")
            elif cell_type == "amount":
                if (
                    hasattr(line_object, "account_group_id")
                    and line_object.account_group_id
                ):
                    cell_format = self.env.format_amount_bold
                else:
                    cell_format = self.env.format_amount
                self.env.sheet.write_number(
                    self.env.row_pos, col_pos, float(value), cell_format
                )
    ...
    def write_line_from_dict(self, line_dict):
        ...
        for col_pos, column in self.env.columns.items():
            ...
            if cell_type == "string":
                ...
                self.env.sheet.write_string(
                    self.env.row_pos, col_pos, value or "", self.env.format_right
                )
        self.env.row_pos += 1
    ...
    def write_initial_balance(self, my_object, label):
        ...
        self.env.sheet.write(
            self.env.row_pos, col_pos_label, label, self.env.format_right
        )
        for col_pos, column in self.env.columns.items():
            if column.get("field_initial_balance"):
                ...
                if cell_type == "string":
                    ...
                    self.env.sheet.write_number(
                        self.env.row_pos, col_pos, float(value), self.env.format_amount
                    )
                elif cell_type == "amount_currency":
    ...
    def write_ending_balance(self, my_object, name, label):
        ...
        Columns are defined with `_get_report_columns` method.
        """
        for i in range(0, len(self.env.columns)):
            self.env.sheet.write(self.env.row_pos, i, "", self.env.format_header_right)
        ...
        self.env.sheet.write(
            self.env.row_pos, col_pos_label, label, self.env.format_header_right
        )
        for col_pos, column in self.env.columns.items():
    ...
    def _get_currency_amt_format(self, line_object):
        ...
        if "account_group_id" in line_object and line_object["account_group_id"]:
            format_amt = self.env.format_amount_bold
            field_prefix = "format_amount_bold"
        else:
            format_amt = self.env.format_amount
        ...
        if "currency_id" in line_object and line_object.get("currency_id", False):
            ...
            if hasattr(self, field_name):
                ...
                format_amt = self.env.workbook.add_format()
    ...
    def _get_currency_amt_header_format(self, line_object):
        ...
        format_amt = self.env.format_header_amount
        if line_object.currency_id:
